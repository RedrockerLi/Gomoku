...
## 训练
在使用位运算时增加棋形不会增加很大的开销,所以可以分出很多种情况.但随之而来的问题是调整各棋形的分数较困难.在这个部分,采用[粒子群算法](https://zhuanlan.zhihu.com/p/346355572)进行调参.

### 淘汰赛
计划使用32个粒子,通过淘汰赛的方式来决出最优者.最符合直觉的记录方式是二叉树,但因为总数已知,采用数组来记录更为方便.

第一个数组为32个元素,用来记录这32个粒子的信息,第二个数组有32+16+8+4+2+1个元素,记录胜者的索引.因为粒子信息占用的内存较大.