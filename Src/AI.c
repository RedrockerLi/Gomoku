...
void value_the_game_with_THREAD_POOL_FOR_AI(ONE_GAME_t * const nowGame_t,ONE_AI_t * const nowAI_t,threadpool *thpoolForAI,int32_t *ans){
    ...
    if(oneGameAndAI.ans!=ans){
        output_log("runningLog","Error:value_the_game_with_THREAD_POOL_FOR_AI ans");
    }
    if(&oneGameAndAI.oneAI_t==nowAI_t){
        output_log("runningLog","Error:value_the_game_with_THREAD_POOL_FOR_AI nowAI_t");
    }
    if(&oneGameAndAI.oneGame_t==nowGame_t){
        output_log("runningLog","Error:value_the_game_with_THREAD_POOL_FOR_AI nowGame_t");
    }
    #endif
...
#endif
    uint8_t i,flag,ansOfCallTheGame;
            ...
            for(uint8_t col=0;col<RANGE_OF_CHESSBOARD;col++){
                if(nowGame_t->stateOfChessboard[MAT(row,col)]==NONE){
                    if(playerFlag==WHITE_PLAYER){
                        ...
                        ansOfCallTheGame=call_the_game(nowGame_t,row,col,1);
                        if(ansOfCallTheGame==BLACK_WINE){
                            scoreOfChild[MAT(row,col)]=MIN_OF_INT32;
                            #ifdef LOG
                            output_log("runningLog","Success:fine_great_children MAX_OR_MIN BLACK_WINE\n");
                            #endif
                        }else if(ansOfCallTheGame==FORBIDDEN_HAND){
                            scoreOfChild[MAT(row,col)]=MAX_OF_INT32;
                            #ifdef LOG
                            output_log("runningLog","Success:fine_great_children MAX_OR_MIN FORBIDDEN_HAND\n");
                            #endif
                        }else{
                        ...
                        if(ansOfCallTheGame==WHITE_WINE){
                            scoreOfChild[MAT(row,col)]=MAX_OF_INT32;
                            #ifdef LOG
                            output_log("runningLog","Success:fine_great_children MAX_OR_MIN WHITE_WINE\n");
                            #endif
                        }else{
                            #ifndef THREAD_POOL_FOR_AI
                            scoreOfChild[MAT(row,col)]=value_the_game(nowGame_t,nowAI_t);
...
int32_t alpha_beta_pruning(ONE_GAME_t * const nowGame_t,ONE_AI_t * const nowAI_t,int8_t depth,int32_t alpha,int32_t beta,uint8_t addRow,uint8_t addCol,threadpool * thpoolForAI){
    ...
    output_log("runningLog",message);
    output_log("runningLog"," addRow:");
    message=int_to_string(addRow);
    output_log("runningLog",message);
    output_log("runningLog"," addCol:");
    ...
    if(ansOfCallTheGame!=CONTINUE){
        if(nowGame_t->playerFlag==BLACK_PLAYER){
            if(ansOfCallTheGame==WHITE_WINE||ansOfCallTheGame==FORBIDDEN_HAND){
                ...
                #ifdef LOG
                output_log("runningLog","Success:alpha_beta_pruning MAX_OR_MIN WHITE_WINE\n");
                #endif
            }else{
                scoreOfChild=MAX_OF_INT32;
                #ifdef LOG
                output_log("runningLog","Success:alpha_beta_pruning MAX_OR_MIN BLACK_WIN\n");
                #endif
            }
        }else{
            if(ansOfCallTheGame==WHITE_WINE||ansOfCallTheGame==FORBIDDEN_HAND){
        ...
        if(depth==-1){
            ...
            if(depth%2==0){//算MIN层节点分数
                ...
                #ifdef LOG
                output_log("runningLog","Success:alpha_beta_pruning->fine_great_children(MIN)\n");
                #endif
                for(i=0,flag=1;flag==1&&i<NUM_OF_CHILDREN;i++){
                    ...
                    #endif
                    #ifdef LOG
                    output_log("runningLog","Success:alpha_beta_pruning->alpha_beta_pruning(MIN)\n");
                    #endif
                ...
                #ifdef LOG
                output_log("runningLog","Success:alpha_beta_pruning->fine_great_children(MAX)\n");
                #endif
                for(i=0,flag=1;flag==1&&i<NUM_OF_CHILDREN;i++){
                    #ifndef THREAD_POOL_FOR_AI
                    ...
                    #endif
                    #ifdef LOG
                    output_log("runningLog","Success:alpha_beta_pruning->alpha_beta_pruning(MAX)\n");
                    #endif
